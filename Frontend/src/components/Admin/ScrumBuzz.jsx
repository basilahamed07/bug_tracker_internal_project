...
const ScrumBuzz = () => {
  ...
  const fetchScrums = async (projectId) => {
    const token = sessionStorage.getItem('access_token');
    try {
      const response = await axios.get(`https://h25ggll0-5000.inc1.devtunnels.ms/agile_details/${projectId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const fetchExistingPIs = async (scrumId) => {
    const token = sessionStorage.getItem('access_token');
    try {
      const response = await axios.get(`https://h25ggll0-5000.inc1.devtunnels.ms/get_pl_name/${scrumId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
