  ...
  const findCurrentPI = async (pis, scrumId, token) => {
    for (const pi of pis) {
      const sprintsResponse = await fetch(
        `https://h25ggll0-5000.inc1.devtunnels.ms/sprint_details/${scrumId}/${pi}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
    ...
    const fetchPIAndSprints = async () => {
      try {
        ...
        const piResponse = await fetch(
          `https://h25ggll0-5000.inc1.devtunnels.ms/get_pl_name/${scrumId}`,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
        ...
        if (pis.length > 0) {
          ...
          if (currentPIData) {
            ...
            const sprintsResponse = await fetch(
              `https://h25ggll0-5000.inc1.devtunnels.ms/sprint_details/${scrumId}/${latestPI}`,
              {
                headers: {
                  'Authorization': `Bearer ${token}`,
                },
...
const fetchScrumName = async () => {
  try {
    ...
    const response = await fetch(
      `https://h25ggll0-5000.inc1.devtunnels.ms/agile_details/${project_name_id}`,
      {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleDeleteClick = async (storyId) => {
    const token = sessionStorage.getItem('access_token');
    try {
      const response = await fetch(`https://h25ggll0-5000.inc1.devtunnels.ms/story_details/${storyId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleDeleteSprintClick = async (sprintId) => {
    ...
    try {
      const token = sessionStorage.getItem('access_token');
      const response = await fetch(
        `https://h25ggll0-5000.inc1.devtunnels.ms/sprint_details/${sprintId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handlePIChange = async (newPI) => {
    try {
      ...
      const response = await fetch(
        `https://h25ggll0-5000.inc1.devtunnels.ms/sprint_details/${scrumId}/${newPI}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
